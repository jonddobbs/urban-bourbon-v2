    ...
    <footer className="bg-[#0d0d0d] border-t border-white/8">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 py-14">
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 mb-10">
          <Link to="/">
            <img src="/images/logo.png" alt="Urban Bourbon" className="h-11 w-auto" />
          </Link>
          ...
          <nav className="flex flex-wrap justify-center gap-6">
            {navLinks.map(({ label, to }) => (
              <Link
                key={to}
                to={to}
                className="text-white/40 hover:text-[#39FF14] font-['Barlow_Condensed'] tracking-[0.12em] uppercase text-sm transition-colors duration-200"
              >
                {label}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-5">
            <a
              href="https://www.facebook.com/urbanbourboncoffee"
              target="_blank"
              rel="noopener noreferrer"
              className="text-white/40 hover:text-[#39FF14] transition-colors duration-200"
              aria-label="Facebook"
            >
              <FacebookIcon />
            </a>
